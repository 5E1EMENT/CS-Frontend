import { RangeType, StrOrNum } from './range.types';

export default class Range<T extends string | number>
  implements Iterable<StrOrNum<T>>
{
  #start: number = 0;
  ...
  reverse(): IterableIterator<StrOrNum<T>> {
    const start =
      this.#rangeType === 'number'
        ? this.#start
        : String.fromCodePoint(this.#start);
    const end =
      this.#rangeType === 'number'
        ? this.#end
        : String.fromCodePoint(this.#end);
  ...
  [Symbol.iterator](): IterableIterator<StrOrNum<T>> {
    ...
    return {
      next: (): IteratorResult<StrOrNum<T>> => {
        if (
          (this.#isReversed && count < this.#end) ||
          (!this.#isReversed && count > this.#end)
        ) {
        ...
        const value = <any>(
          (this.#rangeType === 'number' ? count : String.fromCodePoint(count))
        );
        count += step;
